...
exports.getAvailableCouponsService = exports.validateCouponService = void 0;
 ...
 */
const validateCouponService = async (code, userId, cartTotal, productIds = []) => {
    // 1. Tìm coupon
    const coupon = await coupon_model_1.Coupon.findOne({ code: code.trim().toUpperCase() });
    if (!coupon) {
        throw new ApiError_1.default(404, "Mã giảm giá không tồn tại!");
    }
    // 2. Kiểm tra đang active
    if (!coupon.isActive) {
        throw new ApiError_1.default(400, "Mã giảm giá này đã bị vô hiệu hoá!");
    }
    // 3. Kiểm tra hạn sử dụng
    if (new Date() > coupon.expiresAt) {
        throw new ApiError_1.default(400, "Mã giảm giá đã hết hạn sử dụng!");
    }
    // 4. Kiểm tra còn lượt dùng không
    if (coupon.usageCount >= coupon.maxUsage) {
        throw new ApiError_1.default(400, "Mã giảm giá đã hết lượt sử dụng!");
    }
    ...
    if (userId) {
        ...
        if (alreadyUsed) {
            throw new ApiError_1.default(400, "Bạn đã sử dụng mã giảm giá này rồi!");
        }
    }
    // 6. Kiểm tra đơn tối thiểu
    if (cartTotal < coupon.minOrderAmount) {
        const minFmt = coupon.minOrderAmount.toLocaleString("vi-VN");
        throw new ApiError_1.default(400, `Đơn hàng tối thiểu ${minFmt}đ để dùng mã này!`);
    }
    ...
    if (coupon.applicableProducts.length > 0 && productIds.length > 0) {
        ...
        if (!hasEligibleProduct) {
            throw new ApiError_1.default(400, "Mã giảm giá không áp dụng cho sản phẩm trong giỏ hàng của bạn!");
        }
    }
    // 8. Tính tiền giảm
...
const getAvailableCouponsService = async () => {
    ...
    const coupons = await coupon_model_1.Coupon.find({ isActive: true }).sort({ createdAt: -1 });
...
exports.getAvailableCouponsService = getAvailableCouponsService;

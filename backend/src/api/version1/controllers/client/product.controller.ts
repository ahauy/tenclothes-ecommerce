...
import {
  ...
  getLatestCollectionService,
  getListProductService,
  getProductDetailService,
  getRelatedProductsService,
  searchProductFilterService,
...
export const getListProduct = async (
  req: Request<{ slug?: string }, {}, {}, IRequestQueryFilter>,
  res: Response
): Promise<void> => {
  try {
    ...
    const { products, currentPage, totalPages, totalProducts } =
      await getListProductService(queryFilter, slug);
...
export const getCategoryFilters = async (
  req: Request<{ slug: string }, {}, {}, { keyword?: string }>,
  res: Response
): Promise<void> => {
  try {
    const slug: string = req.params.slug;
    const keyword: string | undefined = req.query.keyword;

    let dynamicFilters;

    if (keyword) {
      dynamicFilters = await searchProductFilterService(keyword);
    } else {
      dynamicFilters = await getCategoryFilterService(slug);
    }
...
export const getRelatedProducts = async (
  req: Request<{}, {}, {}, { slug: string; limit: string }>,
  res: Response
): Promise<void> => {
  try {
...
export const searchProducts = async (
  req: Request<{}, {}, {}, { keyword: string; limit?: string }>,
  res: Response
): Promise<void> => {
  try {
    ...
    const searchResults = await searchProductService(keyword, limit);

...
const Reviews: React.FC = () => {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const [totalRes, pendingRes, flaggedRes] = await Promise.all([
      ...
      setStats({
        ...
        flagged: flaggedRes.data?.pagination?.totalItems || 0,
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("[Reviews Socket] Connected successfully with ID:", socket.id);
    });
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 mb-1">
              ...
              <span className={cn(
                ...
                newReview.status === "pending" ? "text-amber-600 bg-amber-50" : "text-emerald-600 bg-emerald-50"
              )}>
                {newReview.status === "pending" ? "Chờ duyệt" : "Tự động duyệt"}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between mt-2 pt-2 border-t border-neutral-100">
              <span className="text-[10px] font-semibold text-neutral-450">Đánh giá: {newReview.rating}★</span>
              <button
                onClick={() => {
  ...
  const statusOptions = [
    { label: "Tất cả trạng duyệt", value: "all", icon: <Activity className="w-3.5 h-3.5" /> },
    { label: "Chờ phê duyệt", value: "pending", icon: <Info className="w-3.5 h-3.5 text-amber-500" /> },
    { label: "Đã phê duyệt", value: "approved", icon: <Check className="w-3.5 h-3.5 text-emerald-500" /> },
    { label: "Đã từ chối/ẩn", value: "rejected", icon: <X className="w-3.5 h-3.5 text-red-500" /> },
  ];

  const aiOptions = [
    { label: "Tất cả kết quả AI", value: "all", icon: <Shield className="w-3.5 h-3.5" /> },
    { label: "Được AI thông qua", value: "approved", icon: <CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> },
    { label: "AI cảnh báo (Flagged)", value: "flagged", icon: <AlertTriangle className="w-3.5 h-3.5 text-orange-500" /> },
    { label: "AI từ chối (Rejected)", value: "rejected", icon: <XCircle className="w-3.5 h-3.5 text-red-500" /> },
  ];
    ...
    <div className="space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 w-full pb-10 mx-auto">
      ...
      <div className="flex flex-col space-y-1.5 px-2 sm:px-0">
        ...
        <p className="text-neutral-500 font-medium text-sm hidden md:block">
          Kiểm duyệt các phản hồi từ khách hàng và phân tích tự động mức độ an toàn nội dung sử dụng trí tuệ nhân tạo.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 px-2 sm:px-0">
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm flex flex-col justify-between min-h-[120px]">
          <div className="flex items-start justify-between">
            ...
            <span className="text-[9px] font-bold text-neutral-400 bg-neutral-50 px-2 py-1 rounded-md border border-neutral-100">HỆ THỐNG</span>
          </div>
          <div>
            <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1">Tổng đánh giá</p>
            <p className="text-2xl font-bold text-neutral-900 tracking-tight">{stats.total}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm flex flex-col justify-between min-h-[120px]">
          <div className="flex items-start justify-between">
            ...
            <span className="text-[9px] font-bold text-amber-500 bg-amber-50/50 px-2 py-1 rounded-md border border-amber-100/50">YÊU CẦU DUYỆT</span>
          </div>
          <div>
            <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1">Chờ phê duyệt</p>
            <p className="text-2xl font-bold text-neutral-900 tracking-tight">{stats.pending}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm flex flex-col justify-between min-h-[120px]">
          <div className="flex items-start justify-between">
            ...
            <span className="text-[9px] font-bold text-orange-500 bg-orange-50/50 px-2 py-1 rounded-md border border-orange-100/50">CẢNH BÁO TRÍ TUỆ NHÂN TẠO</span>
          </div>
          <div>
            <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1">AI Cảnh báo</p>
            <p className="text-2xl font-bold text-neutral-900 tracking-tight">{stats.flagged}</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:hidden">
          {isLoading && reviews.length === 0 ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div key={`sk-mob-${i}`} className="bg-white p-4 border border-neutral-200 rounded-xl shadow-sm animate-pulse min-h-[200px] flex flex-col gap-3">
              ...
              >
                <div className="flex justify-between items-start border-b border-neutral-100 pb-3 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-[11px] font-bold text-neutral-900 uppercase tracking-widest truncate max-w-[120px]">
                ...
                <div className="space-y-2 mb-4 flex-1">
                  <div className="flex justify-between items-center">
                    ...
                    {review.variantInfo && (
                      <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-wider">
                        Size {review.variantInfo.size} // {review.variantInfo.color}
                      </span>
                    )}
                  </div>
                  ...
                  {review.aiStatus && review.aiStatus !== "approved" && (
                    <div className="flex items-center gap-1.5 bg-orange-50/50 border border-orange-100/60 p-2 rounded-lg mt-2 text-[10px] text-orange-800">
                      ...
                      <span className="truncate">AI: {review.aiReason || "Phát hiện nội dung bất thường"}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-neutral-100 mt-auto">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[9px] font-semibold text-neutral-400">
                      Hữu ích: <span className="font-bold text-neutral-700">{review.isHelpfulCount}</span>
                    </span>
                  </div>
            ...
            <div className="bg-white border border-neutral-200 rounded-xl p-16 text-center col-span-full flex flex-col items-center justify-center">
              ...
              <p className="text-[11px] font-bold text-neutral-400 uppercase tracking-widest">Không tìm thấy đánh giá</p>
            </div>
          )}
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-neutral-50 border-b border-neutral-200">
                <th className="px-6 py-4 text-[10px] font-bold text-neutral-400 uppercase tracking-widest w-[18%]">Khách hàng</th>
                <th className="px-6 py-4 text-[10px] font-bold text-neutral-400 uppercase tracking-widest w-[20%]">Sản phẩm</th>
                <th className="px-6 py-4 text-[10px] font-bold text-neutral-400 uppercase tracking-widest w-[12%]">Đánh giá</th>
                <th className="px-6 py-4 text-[10px] font-bold text-neutral-400 uppercase tracking-widest w-[25%]">Nội dung</th>
                <th className="px-6 py-4 text-[10px] font-bold text-neutral-400 uppercase tracking-widest w-[13%]">Kiểm duyệt AI</th>
                <th className="px-6 py-4 text-[10px] font-bold text-neutral-400 uppercase tracking-widest text-center w-[12%]">Thao tác</th>
              </tr>
            </thead>
              ...
              <AnimatePresence>
                {isLoading && reviews.length === 0 ? (
                  Array.from({ length: 6 }).map((_, i) => (
                    <tr key={`sk-desk-${i}`} className="animate-pulse border-b border-neutral-100 last:border-0">
                    ...
                    >
                      ...
                      <td className="px-6 py-4 align-top">
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="min-w-0">
                            <p className="text-[11px] font-bold text-neutral-900 truncate max-w-[180px]" title={review.productId?.title}>
                      ...
                      <td className="px-6 py-4 align-top">
                        <div className="space-y-1.5">
                          ...
                          <div className="inline-flex">
                            {review.aiStatus === "approved" || !review.aiStatus ? (
                          ...
                          <div className="block">
                            <span className={cn(
                              ...
                              • {review.status === "approved" ? "Đang hiện" : review.status === "pending" ? "Chờ duyệt" : "Đã ẩn"}
                            </span>
                          </div>

                          {review.aiReason && (
                            <p className="text-[9px] text-orange-700 leading-normal line-clamp-1 max-w-[140px]" title={review.aiReason}>
                              {review.aiReason}
                            </p>
                          )}
                        </div>
                      ...
                      <td className="px-6 py-4 text-right align-top" onClick={(e) => e.stopPropagation()}>
                        <div className="flex justify-center items-center gap-1">
                          ...
                          <button
                            onClick={(e) => handleDelete(review._id, e)}
                            disabled={actionInProgress === review._id}
                            className="p-1.5 border border-red-100 bg-red-50 hover:bg-red-100 text-red-650 rounded transition-all shadow-sm"
                            title="Xóa đánh giá vĩnh viễn"
                          >
                            <Trash className="w-3.5 h-3.5" />
                          </button>
      ...
      {totalPages > 1 && (
        <div className="bg-white border border-neutral-200 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm mt-4">
          <p className="text-[11px] text-neutral-500 font-medium">
            Hiển thị <span className="font-bold text-neutral-900">{reviews.length}</span> trên tổng số <span className="font-bold text-neutral-900">{totalElements}</span> đánh giá
          </p>
          <Pagination
            currentPage={page}
            ...
            >
              ...
              <div className="flex justify-between items-center px-8 py-5 border-b border-neutral-100">
                <div className="flex items-center gap-3">
                  <span className="text-[10px] font-medium text-neutral-400 tracking-[0.2em] uppercase">CHI TIẾT ĐÁNH GIÁ</span>
                  <span className="text-neutral-300 font-light">•</span>
                  <span className="font-mono text-[9px] text-neutral-500 bg-neutral-50 px-2 py-0.5 rounded border border-neutral-200/50">MÃ: {selectedReview._id.substring(selectedReview._id.length - 8)}</span>
                </div>
                <button
                  onClick={handleCloseDetail}
              ...
              <div className="p-8 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border border-neutral-200/60 rounded-xl p-4 space-y-3 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                      <h4 className="text-[9px] font-semibold text-neutral-450 uppercase tracking-widest flex items-center gap-1.5">
                        <User className="w-3.5 h-3.5 text-neutral-400" /> Khách hàng
                      </h4>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="overflow-hidden">
                          ...
                          <p className="text-[10px] text-neutral-400 truncate mt-0.5">
                            {selectedReview.userId?.email || "Chưa cập nhật email"}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border border-neutral-200/60 rounded-xl p-4 space-y-3 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                      <h4 className="text-[9px] font-semibold text-neutral-450 uppercase tracking-widest flex items-center gap-1.5">
                        <ShoppingBag className="w-3.5 h-3.5 text-neutral-400" /> Sản phẩm mua
                      </h4>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="overflow-hidden">
                          <p className="text-[11px] font-bold text-neutral-900 line-clamp-2 leading-relaxed">
                            {selectedReview.productId?.title || "Sản phẩm không tồn tại"}
                          </p>
                          {selectedReview.variantInfo && (
                            <p className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest mt-1">
                              Phân loại: size {selectedReview.variantInfo.size} // màu {selectedReview.variantInfo.color}
                            </p>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="space-y-4">
                    <div className={cn(
                      ...
                      (selectedReview.aiStatus === "approved" || !selectedReview.aiStatus) && "bg-emerald-50/10 border-emerald-200/60 text-emerald-800",
                      selectedReview.aiStatus === "flagged" && "bg-orange-50/10 border-orange-200/60 text-orange-800",
                      selectedReview.aiStatus === "rejected" && "bg-red-50/10 border-red-200/60 text-red-800"
                    )}>
                      <div>
                        <h4 className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest flex items-center gap-1.5 mb-2.5">
                          <Shield className="w-3.5 h-3.5 text-neutral-400" /> Kiểm duyệt AI
                        </h4>
                        
                        <div className="flex items-center gap-2 mb-3">
                          {selectedReview.aiStatus === "approved" || !selectedReview.aiStatus ? (
                            <>
                              <CheckCircle className="w-4 h-4 text-emerald-600" />
                              <span className="text-[11px] font-bold uppercase tracking-wider">Nội dung an toàn</span>
                            </>
                          ) : selectedReview.aiStatus === "flagged" ? (
                            <>
                              <AlertTriangle className="w-4 h-4 text-orange-500" />
                              <span className="text-[11px] font-bold uppercase tracking-wider">Cảnh báo nghi ngờ</span>
                            </>
                          ) : (
                            <>
                              <XCircle className="w-4 h-4 text-red-500" />
                              <span className="text-[11px] font-bold uppercase tracking-wider">Vi phạm nội dung</span>
                            </>
                          )}
                        </div>
                        ...
                        {selectedReview.aiReason ? (
                          ...
                          <p className="text-[11px] text-neutral-500 bg-white/70 rounded-lg p-3.5 border border-neutral-100">
                            Không phát hiện nội dung độc hại hoặc nhạy cảm trong đánh giá này.
                          </p>
                        )}
                      </div>
                      ...
                      <p className="text-[8px] text-neutral-400 font-medium tracking-wide">
                        *Kiểm duyệt tự động dựa trên mô hình ngôn ngữ và danh sách từ cấm.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-y border-neutral-100 py-6 flex flex-wrap justify-between items-center gap-6">
                  <div className="flex items-center gap-6">
                    <div>
                      <span className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest block mb-1">Điểm số</span>
                      <div className="flex items-center gap-2.5">
                        <span className="text-4xl font-extralight text-neutral-900 tracking-tight leading-none">{selectedReview.rating.toFixed(1)}</span>
                    ...
                    <div>
                      <span className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest block mb-1">Tương tác</span>
                      <span className="text-[11px] font-semibold text-neutral-500">
                        Có <span className="font-bold text-neutral-800">{selectedReview.isHelpfulCount}</span> lượt bình chọn hữu ích
                      </span>
                    </div>
                  </div>

                  <div>
                    <span className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest block mb-1 text-right">Ngày gửi</span>
                ...
                <div className="space-y-3">
                  <h4 className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest">Nội dung đánh giá</h4>
                ...
                {selectedReview.images && selectedReview.images.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest">Hình ảnh đính kèm ({selectedReview.images.length})</h4>
              ...
              <div className="px-8 py-5 border-t border-neutral-100 bg-neutral-50/50 flex justify-between items-center mt-auto">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest">TRẠNG THÁI HIỂN THỊ:</span>
                  <span className="text-[9px] font-bold uppercase tracking-wider rounded-md px-2.5 py-0.5 border border-neutral-200 bg-white text-neutral-600 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                    {selectedReview.status === "approved" ? "Hiển thị" : selectedReview.status === "pending" ? "Chờ duyệt" : "Đã bị ẩn"}
                  </span>
                </div>
            ...
            <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 pointer-events-none">
              ...
              >
                ...
                <div className="flex items-center gap-3.5 mb-4">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-neutral-900 tracking-tight">Xác nhận xóa đánh giá</h3>
                    <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">Không thể hoàn tác</p>
                  </div>
                </div>

                {/* Content */}
                {(() => {
                  const targetReview = reviews.find(r => r._id === deleteTargetId) || (selectedReview?._id === deleteTargetId ? selectedReview : null);
                  return (
                    <>
                      <p className="text-[12px] text-neutral-600 leading-relaxed font-medium">
                        ...
                        ? Hệ thống sẽ cập nhật lại điểm đánh giá trung bình của sản phẩm tương ứng.
                      </p>
                      ...
                      {targetReview && (
                        <div className="bg-neutral-50 border border-neutral-100 rounded-xl p-3.5 my-4 space-y-2 text-left">
                          <div className="flex justify-between items-center">
                            <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest">Nội dung đánh giá</span>
                            <div className="flex items-center gap-0.5">
                              {Array.from({ length: 5 }).map((_, i) => (
                                <Star
                                  key={i}
